...
--  get all the infromation from customers who placed an order
SELECT DISTINCT C.CustomerID, C.FirstName, C.LastName, C.Phone, C.Email
FROM Customers C
JOIN Orders O ON C.CustomerID = O.CustomerID;

--  get the details of all orders placed on a september 2, 2024
SELECT O.OrderID, C.FirstName, C.LastName, O.OrderDate, O.TotalAmount
FROM Orders O
JOIN Customers C ON O.CustomerID = C.CustomerID
WHERE O.OrderDate = '2024-09-02';
